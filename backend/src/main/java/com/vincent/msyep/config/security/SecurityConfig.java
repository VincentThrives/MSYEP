    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/api/v1/zones/**").hasAnyRole("SUPER_ADMIN", "ADMIN", "ZONE", "STAFF")
                .requestMatchers(HttpMethod.GET, "/api/v1/centers/**").authenticated()
                .requestMatchers("/api/v1/centers/**").hasAnyRole("SUPER_ADMIN", "ADMIN", "ZONE", "CENTER", "STAFF")
                .requestMatchers(HttpMethod.GET, "/api/v1/students/**").authenticated()
                // A student may maintain their OWN profile: edit their record and upload their
                // documents. Ownership (id == their own) is enforced in the controller.
                .requestMatchers(HttpMethod.PUT, "/api/v1/students/*").hasAnyRole("SUPER_ADMIN", "ADMIN", "ZONE", "CENTER", "STUDENT")
                .requestMatchers(HttpMethod.POST, "/api/v1/students/*/documents").hasAnyRole("SUPER_ADMIN", "ADMIN", "ZONE", "CENTER", "STUDENT")
                .requestMatchers("/api/v1/students/**").hasAnyRole("SUPER_ADMIN", "ADMIN", "ZONE", "CENTER")
                .requestMatchers("/api/v1/finance/**").hasAnyRole("SUPER_ADMIN", "ADMIN", "FINANCE", "ZONE", "CENTER", "STAFF")
